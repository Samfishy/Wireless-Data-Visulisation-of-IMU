...
void nrf24_int32_array_to_bytes(const int32_t *in, uint8_t *out, uint8_t count)
{
    ...
    {
        uint32_t val = (uint32_t)in[i];   /* reinterpret bits, no sign issue */
        out[i*4 + 0] = (uint8_t)((val >>  0) & 0xFFU);
        out[i*4 + 1] = (uint8_t)((val >>  8) & 0xFFU);
        out[i*4 + 2] = (uint8_t)((val >> 16) & 0xFFU);
        out[i*4 + 3] = (uint8_t)((val >> 24) & 0xFFU);
    }
}
